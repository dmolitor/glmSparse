...
library(dplyr)
library(readr)
pums_dat <- read_delim("~/Documents/data/USCensus1990.data.txt", 
                       delim = ",", 
                       n_max = 50000) %>%
  select(-caseid) %>%
...
In this example, I will compare `glmSparse` to `glm` and `speedglm` using a 
sub-sample of the [PUMS data for the 1990 census]
(https://www.census.gov/data/datasets/1990/dec/pums.html) to
predict an individual's sex. This simplified version of the data is a good
example of a case where there will be significant sparsity, as each variable
...
```{r Compare glm with glmSparse and speedglm, echo=FALSE, message=FALSE, warning=FALSE}
benchmarks <- mark(
  (function() {
    glm(iSex ~ .,
